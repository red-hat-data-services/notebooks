...
# Install s390x-specific build dependencies
if [[ $(uname -m) == "s390x" ]]; then
    # Install base development packages
    dnf install -y perl mesa-libGL skopeo libxcrypt-compat python3.12-devel pkgconf-pkg-config gcc gcc-gfortran gcc-c++ ninja-build make openssl-devel python3-devel pybind11-devel autoconf automake libtool cmake pkg-config openblas-devel libjpeg-devel zlib-devel libtiff-devel freetype-devel lcms2-devel libwebp-devel git tar wget unzip python3.12-pip python3.12-setuptools unixODBC-devel && \
    # Install EPEL repository
    dnf install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-9.noarch.rpm && \
    # Install additional development tools from EPEL
    dnf install -y fribidi-devel lcms2-devel openjpeg2-devel libraqm-devel libimagequant-devel tcl-devel tk-devel && \
...
if [[ $(uname -m) == "ppc64le" ]] || [[ $(uname -m) == "s390x" ]]; then
    CURDIR=$(pwd)
     ...
     # install development packages (match Runtime DataScience)
     dnf install -y https://dl.fedoraproject.org/pub/epel/epel-release-latest-9.noarch.rpm
