...
# RHDS / RHOAI (required for this image) — BUILD_ARCH must match the platform you will build
...
This image is **RHDS-only**: Tekton prefetches `prefetch-input/rhds` and builds with
`build-args/konflux.cpu.conf`. Local builds should use `PRODUCT=rhoai`:
...
4. RPMs (gcc, nodejs, nginx, etc. via Hermeto from RHEL repos)
