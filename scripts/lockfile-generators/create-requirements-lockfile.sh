...
# Hybrid codeserver (rhoai-2.25 / rhoai-3.3): lock and install against public PyPI so GHA
# prefetch does not regenerate requirements from the AIPCC RH index (missing pkgs like bigtree).
# RH wheels (ripgrep/uv/ppc64le) stay as direct public-rhai URLs in requirements.cpu.txt.
...
if [[ -f "$RH_WHEEL_REF" ]]; then
  ...
  # Rust sdists / missing matching-version BE wheels — replace on all arches.
  ...
  # (version-matched entries in rh-wheel-only.ref.toml).
  RH_WHEEL_MERGE_BE=(
...
PY
  ...
  RH_WHEEL_REPLACE_PKGS=()
  while IFS= read -r pkg; do
    [[ -n "$pkg" ]] && RH_WHEEL_REPLACE_PKGS+=("$pkg")
  done < <(rh_wheel_packages_in_ref "$RH_WHEEL_REF" "${RH_WHEEL_REPLACE_ALL[@]}")
  RH_WHEEL_MERGE_PKGS=()
  while IFS= read -r pkg; do
    [[ -n "$pkg" ]] && RH_WHEEL_MERGE_PKGS+=("$pkg")
  done < <(rh_wheel_packages_in_ref "$RH_WHEEL_REF" "${RH_WHEEL_MERGE_BE[@]}")
  if [[ ${#RH_WHEEL_REPLACE_PKGS[@]} -gt 0 ]]; then
    python3 "${SCRIPTS_PATH}/helpers/patch-rh-wheel-only-packages.py" \
        replace "$PYLOCK_FILE" "$RH_WHEEL_REF" \
        "${RH_WHEEL_REPLACE_PKGS[@]}"
  fi
  if [[ ${#RH_WHEEL_MERGE_PKGS[@]} -gt 0 ]]; then
    python3 "${SCRIPTS_PATH}/helpers/patch-rh-wheel-only-packages.py" \
        merge-be "$PYLOCK_FILE" "$RH_WHEEL_REF" \
        "${RH_WHEEL_MERGE_PKGS[@]}"
  else
    echo "  Skipping merge-be (no BE overlay packages in ${RH_WHEEL_REF})"
  fi
fi
