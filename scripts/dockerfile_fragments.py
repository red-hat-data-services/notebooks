...
def main():
    ...
    replacements = {
        ...
        "upgrade first to avoid fixable vulnerabilities": textwrap.dedent(r"""
            ...
            # RHAIENG-6037: the activation key is EUS 9.6 — pin to RHEL 9 GA repos.
            if command -v subscription-manager &> /dev/null; then
              if subscription-manager identity &>/dev/null; then
                ...
                subscription-manager repos --disable='*' \
                  --enable="rhel-9-for-${basearch}-baseos-rpms" \
                  --enable="rhel-9-for-${basearch}-appstream-rpms" \
                  --enable="codeready-builder-for-rhel-9-${basearch}-rpms"
              else
