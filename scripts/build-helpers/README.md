...
| Variant | Subscription | Make target | Dockerfile |
|---------|--------------|-------------|------------|
| `c9s` | No | `rstudio-c9s-python-3.11` | `Dockerfile.cpu` |
| `rhel9` | Yes | `rstudio-rhel9-python-3.11` | `Dockerfile.cpu` |

Both variants use the existing Makefile targets and Dockerfiles already in this repo. The helper adds subscription setup, entitlement mounts, and local run commands — it does not change the image recipes.

RHEL9 local builds mount subscription entitlements into `podman build` via `CONTAINER_BUILD_CACHE_ARGS`.
