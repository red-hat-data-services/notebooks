...
**RHOAI 3.3** workbenches. Builds are **hybrid-hermetic** (same model as rhoai-2.25): npm is
prefetched for the large VS Code tree; Python wheels, X.org sources, and
`oc` are installed over the network at build time to avoid Conforma
`hermeto:pip:package:binary` / generic SBOM failures. Konflux uses
`hermetic=false` with npm-only `prefetch-input`. AIPCC CPU base images
are kept for Konflux (`build-args/konflux.cpu.conf`).
...
| VS Code | **1.112.0** |
| Node.js (RPM) | **nodejs:22** module (Konflux/RHEL 9.6 may ship 22.19.x; UBI/GHA locks may be newer). `VSCODE_SKIP_NODE_VERSION_CHECK=1` allows builds when the RPM lags VS Code `.nvmrc`. |
...
│   ├── rhds/                    # Lockfiles used on rhoai-3.3 (--rhds prefetch)
...
**rhoai-3.3** there are two outputs — one for CI/online builds, one for hermetic
...
# rhoai-3.3: use --rhds (public UBI + CentOS repos; no subscription required)
...
│   ├── artifacts.in.yaml
│   └── artifacts.lock.yaml
├── rhds/            # downstream / rhoai-3.3 lockfiles (public UBI + CentOS Stream)
...
|----------|----------|-------------|
| `BASE_IMAGE` | Yes | Base image (`registry.access.redhat.com/ubi9/python-312:latest` on rhoai-3.3) |
