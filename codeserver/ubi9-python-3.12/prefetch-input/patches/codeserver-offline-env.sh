...
# libxkbfile built from X.org sources (install-xkbfile-from-generic.sh). PKG_CONFIG_PATH
# must be re-exported in every RUN that builds native-keymap (env from the install
# RUN does not persist). Needed especially on ppc64le/s390x where .pc may live under
# /usr/lib/pkgconfig rather than the default search path.
...
export npm_config_nodedir=/usr
# RHEL 9.6 nodejs:22 (e.g. 22.19.0) can lag VS Code's .nvmrc (22.22.0+).
# Upstream preinstall.ts honors this env var; without it npm ci fails on Konflux.
