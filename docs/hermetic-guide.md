...
3. **Resource limits**  GHA runners have 4 vCPUs and 16 GB RAM, which is
   ...
   `--env=NODE_OPTIONS=--max-old-space-size=1536 --env=JOBS=2` via
   `CONTAINER_BUILD_CACHE_ARGS` for codeserver targets only. This caps the
   V8 heap to 1.5 GB per process and limits parallel compilation to 2 jobs
   (~8.5 GB total, well within the 16 GB runner).
...
- **Resource limits**  `--env=NODE_OPTIONS=--max-old-space-size=1536` and
  `--env=JOBS=2` injected for codeserver via `CONTAINER_BUILD_CACHE_ARGS`.
